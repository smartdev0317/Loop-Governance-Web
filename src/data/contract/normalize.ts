...
import { govStakerQuery, userStakedQueryquery, userStakableQueryquery } from "./contract"
...
export const userStakedQuery = selector({
  key: "userStaked",
  get: ({ get }) => get(userStakedQueryquery)?.toString() ?? "0",
})

export const userStakableQuery = selector({
  key: "userStakable",
  get: ({ get }) => get(userStakableQueryquery)?.balance?.toString() ?? "0",
})
...
const userStakableState = atom ({
  key: "userStakableState",
  default: "0",
})
...
export const useUserStaked = () => {
  return useStore(userStakedQuery, userStakedState)
}

export const useUserStakable = () => {
  return useStore(userStakableQuery, userStakableState)
}
