...
import { /*useFindBalance,*/ useUserStakable } from "../data/contract/normalize"
...
const CreatePollForm = ({ type, headings }: Props) => {
  ...
  const { contents: userStakable } = useUserStakable()
  ...
  const { mint, collateralOracle } = contracts
  const { factory, community, oracleHub } = contracts

  const networkName = useRecoilValue(networkNameState)

  let _loopToken = loopToken.mainnet.token
  let gov = loopGovernanceContract.mainnet

  if (networkName === "mainnet") {}
  else if (networkName === "testnet") {
    _loopToken = loopToken.testnet.token
    gov = loopGovernanceContract.testnet
  }
    ...
    return record(
      {
        ...
        [Key.proposalDeposit]: !proposalDeposit
          ? ""
          : v.amount(proposalDeposit, { symbol: "LOOP" }),
        ...
        [Key.amount]: v.amount(amount, { symbol: "LOOP" }),
      },
      "",
      getFieldKeys()
    )
  ...
  const selectAssetConfig: Config = {
    ...
    validate: isCollateral ? undefined : ({ symbol }) => symbol !== "LOOP",
  ...
  const configPlaceholders = {
    ...
    [Key.proposalDeposit]:
      lookup(config?.default_poll_config.proposal_deposit, "LOOP") ?? "",
  ...
  const fields = {
    deposit: {
      help: renderBalance(userStakable, "LOOP"),
      label: <TooltipIcon content={Tooltips.Gov.Deposit}>Deposit</TooltipIcon>,
      value: <Formatted symbol="LOOP">{deposit}</Formatted>,
    },
    ...
    ...getFields({
      ...
      [Key.amount]: {
        ...
        input: { placeholder: placeholder("LOOP") },
        help: renderBalance(spend_limit, "LOOP"),
        unit: "LOOP",
        unitAfterValue: true,
      },
    }),
  ...
  const data = [
    newContractMsg(_loopToken, {
      send: { amount: deposit, contract: gov, msg },
    }),
  ]

  const messages = !gte(userStakable, deposit)
